def generate_takeaways_and_critique(ratios, parsed=None):
    ...
    if parsed:
        ...
        def format_rupiah(val):
            ...
            sign = "-" if val < 0 else ""
            if abs_val >= 1000:
                return f"Rp {sign}{abs_val/1000:,.2f} M"
            else:
                return f"Rp {sign}{abs_val:,.0f} Juta"
        ...
        def scan_anomalies(data_dict, category_name):
            for item_name, item_data in data_dict.items():
                ...
                if val1 != 0:
                    ...
                    # Data dalam satuan juta Rupiah
                    # Threshold: > 25% perubahan DAN > Rp 1 Miliar (= 1000 dalam satuan juta)
                    if abs(pct_change) >= 0.25 and abs(diff) >= 1000:
    ...
    kritis.append(
        f"**K1: Tekanan Klaim Ta'widh**: Beban klaim (Ta'widh bruto) tumbuh sebesar {uw.get('yoy_gross_claims_pct', 0):.1f}% YoY "
        f"(dari Rp {uw.get('gross_claims_prev_juta', 0)/1000:.1f} M ke Rp {uw.get('gross_claims_juta', 0)/1000:.1f} M), "
        f"dibandingkan dengan laju pertumbuhan pendapatan (IJK Bruto) sebesar {uw.get('yoy_ijk_bruto_pct', 0):.1f}%."
    )
    
    # K2: Kemampuan Penagihan
    if uw.get('yoy_net_recoveries_pct', 0) < 0:
        kritis.append(
            f"**K2: Kemampuan Pemulihan (Recoveries) Anjlok**: Penerimaan pemulihan hak tagih (subrogasi kas) turun sebesar {abs(uw.get('yoy_net_recoveries_pct', 0)):.1f}% YoY "
            f"(dari Rp {uw.get('net_recoveries_prev_juta', 0)/1000:.1f} M ke Rp {uw.get('net_recoveries_juta', 0)/1000:.1f} M), "
    ...
    if uw.get('yoy_claims_receivable_pct', 0) > 0:
        kritis.append(
            f"**K3: Akumulasi Piutang Ta'widh**: Saldo Piutang Ta'widh (klaim dibayar namun belum tertagih dari debitur) meningkat {uw.get('yoy_claims_receivable_pct', 0):.1f}% YoY. "
    ...
    if conc.get('top3_share_pct', 0) > 50:
        kritis.append(
            f"**K4: Risiko Konsentrasi Tinggi**: Sebesar {conc.get('top3_share_pct', 0):.1f}% dari total portofolio Kafalah "
            f"terkonsentrasi pada 3 mitra utama, dengan mitra terbesar menguasai {conc.get('bsi_share_pct', 0):.1f}%."
        )
    ...
    if prof.get('yoy_net_profit_growth_pct', 0) < 0:
        perhatian.append(
            f"**P1: Penurunan Laba Bersih**: Laba bersih turun sebesar {abs(prof.get('yoy_net_profit_growth_pct', 0)):.1f}% YoY "
            f"(dari Rp {prof.get('net_profit_prev_juta', 0)/1000:.1f} M ke Rp {prof.get('net_profit_juta', 0)/1000:.1f} M) meskipun pendapatan (IJK) mengalami pertumbuhan."
        )
    ...
    if headroom_pct > 70:
        perhatian.append(
            f"**P2: Kapasitas Gearing Mendekati Batas**: Gearing ratio aktual mencapai {gearing_ratio:.2f}x, "
            f"mewakili {headroom_pct:.1f}% dari batas maksimum POJK sebesar {limit}x. Sisa ruang ekspansi penjaminan neto "
            f"tercatat sebesar Rp {solv.get('additional_capacity_triliun', 0):.2f} Triliun."
        )
    ...
    perhatian.append(
        f"**P3: Serapan Anggaran Beban Usaha**: Serapan anggaran Beban Usaha mencapai {opex_pct:.1f}% YTD "
        f"(Rp {uw.get('total_opex_juta', 0)/1000:.1f} M dari plafon RKAP Rp {uw.get('total_opex_rkap_juta', 0)/1000:.1f} M)."
    )
    ...
    if uw.get('expense_ratio', 100) < 60:
        positif.append(
            f"**H1: Efisiensi Beban Operasional**: Rasio beban (expense ratio) tercatat sangat efisien di level {uw.get('expense_ratio', 0):.1f}%."
        )
    ...
    if invest_share_sbsn > 50:
        positif.append(
            ...
            f"yaitu Surat Berharga Syariah Negara (SBSN) sebesar {invest_share_sbsn:.1f}%."
        )
        
    if prof.get('rkap_ytd_achieved_pct', 0) > 90:
        positif.append(
            f"**H3: Pencapaian Laba Kuat**: Realisasi laba telah mencapai {prof.get('rkap_ytd_achieved_pct', 0):.1f}% dari target anggaran YTD."
        )
    ...
    critical_analysis['kinerja_underwriting'] = (
        ...
        f"1. **Tingkat Klaim**: Rasio Kerugian Neto (Net Loss Ratio) tercatat di level {uw.get('loss_ratio', 0):.2f}%. Tingkat ini masuk kategori {loss_ratio_desc}. "
        f"Beban klaim bruto mencapai Rp {uw.get('gross_claims_juta', 0)/1000:.1f} M.\n"
        f"2. **Rasio Gabungan (Combined Ratio)**: Ditopang oleh Beban Usaha sebesar Rp {uw.get('total_opex_juta', 0)/1000:.1f} M, "
        f"Rasio Gabungan berada di tingkat {uw.get('combined_ratio', 0):.2f}%. Laba teknik underwriting yang dihasilkan "
        f"sebesar Rp {uw.get('net_uw_income_juta', 0)/1000:.1f} M."
    )
    ...
    critical_analysis['kinerja_profitabilitas'] = (
        ...
        f"1. **Tren Laba Bersih**: Realisasi Laba Bersih sebesar Rp {prof.get('net_profit_juta', 0)/1000:,.1f} M "
        f"{profit_trend} sebesar {prof.get('yoy_net_profit_growth_pct', 0):.1f}% YoY.\n"
        f"2. **Pencapaian Target RKAP**: Kinerja profitabilitas mencapai {prof.get('rkap_fy_achieved_pct', 0):.2f}% dari target Full Year. "
        f"Hasil investasi menyumbang Rp {prof.get('investment_income_juta', 0)/1000:,.1f} M dengan pertumbuhan YoY sebesar {prof.get('yoy_invest_income_growth_pct', 0):.1f}%."
    )
    
    # Risiko Konsentrasi
    critical_analysis['risiko_konsentrasi'] = (
        "**Analisis Risiko Konsentrasi Portofolio Mitra**:\n"
        f"1. **Indeks Konsentrasi**: Konsentrasi portofolio dengan HHI Index mencapai {conc.get('hhi_index', 0):,.0f}. "
        f"Sebesar {conc.get('top3_share_pct', 0):.2f}% dari total outstanding dikuasai oleh 3 mitra teratas.\n"
    ...
    critical_analysis['solvabilitas_regulasi'] = (
        ...
        f"1. **Pemanfaatan Gearing Aktual**: Gearing ratio aktual sebesar {solv.get('gearing_ratio', 0):.2f}x.\n"
        f"2. **Sisa Gearing Headroom**: Sisa ruang ekspansi adalah sebesar {solv.get('headroom', 0):.2f}x, setara dengan tambahan kapasitas penerbitan penjaminan "
        f"sebesar Rp {solv.get('additional_capacity_triliun', 0):.2f} Triliun."
    )
    
    # 5. Ekuitas
    dupont_analysis = (
        "**Analisis Struktur Ekuitas**:\n"
        f"Perusahaan mencatatkan ekuitas sebesar Rp {solv.get('equity_juta', 0)/1000:,.1f} M untuk menopang total outstanding "
        f"Kafalah bersih sebesar Rp {solv.get('os_net_juta', 0)/1000:,.1f} M."
    )
